...
export default function EditBilty({ params }: { params: { id: string } }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json()
        alert(data.error || "Failed to update bilty")
      }
    } catch (error) {
      console.error("Error updating bilty:", error)
      alert("Failed to update bilty")
    }
  }

  return (
    <div className="container mx-auto px-4 pt-24 py-8">
      <h1 className="text-2xl font-bold mb-6">Edit Bilty</h1>
